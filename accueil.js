/* rdr-elements accueil | source route-du-rhum 74f26ba | home-news.js skippers-showcase.js rdr-espace-promo.js partners-grid.js */
try{(window.RDR_ELEMENTS=window.RDR_ELEMENTS||{})["accueil"]="74f26ba";performance.mark("rdr-elements:accueil")}catch(e){}
;(function(){
...
;(function(){
if (!customElements.get('partners-grid')) {

function wixToUrl(v) {
  if (!v) return '';
  if (v.startsWith('http')) return v;
  const mImg = v.match(/wix:image:\/\/v1\/([^/#]+)/);
  if (mImg) return 'https://static.wixstatic.com/media/' + mImg[1];
  const mVec = v.match(/wix:vector:\/\/v1\/([^/#]+)/);
  if (mVec) return 'https://static.wixstatic.com/shapes/' + mVec[1];
  return v;
}
...
const CATS_FR = {
  'Majeur Exclusif':         { label: 'PARTENAIRE MAJEUR EXCLUSIF',  h: 190 },
  'Principaux':              { label: 'PARTENAIRES PRINCIPAUX',       h: 138 },
  'Officiels':               { label: 'PARTENAIRES OFFICIELS',        h: 124 },
  'Médias':                  { label: 'PARTENAIRES MÉDIAS',           h: 124 },
  'Fournisseurs Officiels':  { label: 'FOURNISSEURS OFFICIELS',       h: 105 },
  'Fournisseurs Techniques': { label: 'FOURNISSEURS TECHNIQUES',      h: 100 },
  'En collaboration avec':   { label: 'AVEC LE SOUTIEN DE',           h: 100 },
  'Un événement':            { label: 'UN ÉVÉNEMENT',                 h: 100 },
};

const CATS_EN = {
  'Majeur Exclusif':         { label: 'EXCLUSIVE MAJOR PARTNER',     h: 190 },
  'Principaux':              { label: 'MAIN PARTNERS',               h: 138 },
  'Officiels':               { label: 'OFFICIAL PARTNERS',           h: 124 },
  'Médias':                  { label: 'MEDIA PARTNERS',              h: 124 },
  'Fournisseurs Officiels':  { label: 'OFFICIAL SUPPLIERS',          h: 105 },
  'Fournisseurs Techniques': { label: 'TECHNICAL SUPPLIERS',         h: 100 },
  'En collaboration avec':   { label: 'WITH THE SUPPORT OF',         h: 100 },
  'Un événement':            { label: 'AN EVENT',                    h: 100 },
};

const ROWS = [
  { keys: ['Majeur Exclusif', 'Principaux'],                                    fullWidth: false },
  { keys: ['Officiels'],                                                         fullWidth: true  },
  { keys: ['Médias'],                                                            fullWidth: true  },
  { keys: ['Fournisseurs Officiels'],                                            fullWidth: true  },
  ...
  { keys: ['Fournisseurs Techniques'], fullWidth: true },
  { keys: ['En collaboration avec', 'Un événement'], fullWidth: false },
];
...
const DELAI_SECOURS = 1500;
const URL_SECOURS = '/_functions/partenaires';
let _secoursEnVol = null;    

function chargerPartenaires() {
  if (!_secoursEnVol) {
    _secoursEnVol = fetch(URL_SECOURS, { credentials: 'same-origin' })
      .then(r => (r.ok ? r.json() : null))
      .then(j => (j && Array.isArray(j.partners) ? j.partners : []))
      .catch(() => { _secoursEnVol = null; return []; });
  }
  return _secoursEnVol;
}
...
function langueDeLaPage() {
  try {
    ...
    return /^en\b/i.test(document.documentElement.lang || '') ? 'en' : 'fr';
  } catch (e) { return 'fr'; }
}



function signature(partners, lang) {
  return lang + '::' + partners.map(p => [p.logoBlanc, p.logoAltText, (Array.isArray(p.type) ? p.type : [p.type]).join(','), p.link, p.logoWidth, p.ordre].join('|')).sort().join('\n');
}

class PartnersGrid extends HTMLElement {
  constructor() {
    super();
    ...
    this._filet = null;        
    this._secours = null;      
    this._donneesSecours = null;
  }
  static get observedAttributes() { return ['partners', 'lang']; }
  ...
  attributeChangedCallback() { this._renderSiNouveau(); }
  connectedCallback() {
    this._renderSiNouveau();
    if (this._secours) clearTimeout(this._secours);
    this._secours = setTimeout(() => { this._secours = null; this._secourir(); }, DELAI_SECOURS);
  }
  _secourir() {
    if (!this.isConnected || this.getAttribute('partners')) return;
    chargerPartenaires().then(partners => {
      if (!partners.length || !this.isConnected || this.getAttribute('partners')) return;
      this._donneesSecours = partners;
      this._renderSiNouveau();
    });
  }
  disconnectedCallback() {
    if (this._secours) { clearTimeout(this._secours); this._secours = null; }
     
     
    if (this._scrollAbort) { this._scrollAbort.abort(); this._scrollAbort = null; }
    if (this._observer) { this._observer.disconnect(); this._observer = null; }
     
    if (this._filet) { clearTimeout(this._filet); this._filet = null; }
     
     
    this._derniereCle = null;
  }
  _renderSiNouveau() {
    ...
    let partners = null;
    const brut = this.getAttribute('partners');
    if (brut) {
      try { partners = JSON.parse(brut); } catch(e) { partners = null; }
    }
    if (!Array.isArray(partners) || !partners.length) partners = this._donneesSecours;
    if (!Array.isArray(partners) || !partners.length) return;
    const lang = this.getAttribute('lang') || langueDeLaPage();
    const cle = signature(partners, lang);
    if (cle === this._derniereCle) return;
    this._derniereCle = cle;
    this._render(partners, lang);
  }

  _render(partners, lang) {
    const CATS = lang === 'en' ? CATS_EN : CATS_FR;

    const get = key => partners
      .filter(p => {
        const tags = Array.isArray(p.type) ? p.type : (p.type ? [p.type] : []);
        return tags.includes(key);
      })
      .sort((a, b) => (a.ordre ?? 999) - (b.ordre ?? 999));

    const makeLogos = (items, h) => items.map(p => {
      const url = isSafeUrl(wixToUrl(p.logoBlanc));
      



      const alt = escAttr(p.logoAltText || 'Partenaire');
      const link = isSafeUrl(p.link || '');
      const hm = Math.round(h * 0.78);
      const img = `<img src="${url}" alt="${alt}" loading="lazy" decoding="async" class="pg-img" style="width:${h}px;height:${h}px;object-fit:contain;--hm:${hm}px;">`;
      return link
        ? `<a href="${link}" class="pg-item" target="_blank" rel="noopener" aria-label="${alt}">${img}</a>`
        : `<span class="pg-item">${img}</span>`;
    }).join('');

    const rowsHTML = ROWS.map(row => {
      const filled = row.keys.filter(k => get(k).length > 0);
      if (!filled.length) return '';
      const blocksHTML = filled.map(key => {
        const cat = CATS[key];
        const items = get(key);
        const wrapClass = row.fullWidth ? 'pg-logos wrap' : 'pg-logos';
        return `
<div class="pg-block">
  <div class="pg-label"><span class="pg-label-inner">${cat.label}</span></div>
  <div class="${wrapClass}">${makeLogos(items, cat.h)}</div>
</div>`;
      }).join('');
      return `<div class="pg-row${row.fullWidth ? ' full' : ''}">${blocksHTML}</div>`;
    }).filter(Boolean).join('\n');
...
<style>
  ...
  partners-grid { display: block; width: 100%; }
  partners-grid, partners-grid *, partners-grid *::before, partners-grid *::after { box-sizing: border-box; margin: 0; padding: 0; }
  .pg { width: 100%; font-family: 'Montserrat', sans-serif; }
  .pg-row { display: flex; flex-wrap: wrap; width: 100%; }
  ...
  .pg-block { flex: 1 1 200px; min-width: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 18px; padding: 22px 16px; }
  .pg-label { display: flex; align-items: center; gap: 12px; width: 100%; justify-content: center; }
  .pg-label::before, .pg-label::after { content: ''; flex: 1; max-width: 56px; height: 1px; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.32)); transition: background 0.35s ease; }
  .pg-label::after { background: linear-gradient(270deg, transparent, rgba(255,255,255,0.32)); }
  .pg-label-inner { font-size: 9px; font-weight: 500; letter-spacing: 0.28em; text-transform: uppercase; color: rgba(255,255,255,0.72); white-space: nowrap; flex-shrink: 0; transition: color 0.35s ease; }
  .pg-block:hover .pg-label-inner { color: rgba(255,255,255,0.85); }
  .pg-block:hover .pg-label::before { background: linear-gradient(90deg, transparent, rgba(252,220,80,0.5)); }
  .pg-block:hover .pg-label::after { background: linear-gradient(270deg, transparent, rgba(252,220,80,0.5)); }
  ...
  .pg-logos { display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 10px; }
  .pg-logos.wrap { gap: 12px 20px; }
  .pg-item { display: inline-flex; align-items: center; justify-content: center; text-decoration: none; line-height: 0; flex-shrink: 0; }
  .pg-img { display: block; object-fit: contain; opacity: 0; transform: translateY(10px); transition: opacity 0.5s ease, transform 0.5s cubic-bezier(0.16,1,0.3,1), filter 0.28s ease, scale 0.28s cubic-bezier(0.34,1.56,0.64,1); }
  .pg-img.visible { opacity: 0.82; transform: translateY(0); }
  ...
  @media (hover: hover) and (pointer: fine) {
    .pg-logos:hover .pg-img { opacity: 0.28; }
    .pg-logos:hover .pg-item:hover .pg-img { opacity: 1; scale: 1.1; filter: drop-shadow(0 6px 18px rgba(255,255,255,0.22)); }
  }
  @media (max-width: 640px) {
    .pg-block { flex-basis: 100%; padding: 12px 10px; gap: 12px; }
    .pg-logos { flex-wrap: wrap !important; gap: 6px 8px !important; }
    .pg-logos.wrap { gap: 6px 10px !important; }
    .pg-img { width: var(--hm) !important; height: var(--hm) !important; }
    .pg-label-inner { white-space: normal; text-align: center; }
    .pg-label::before, .pg-label::after { max-width: 20px; }
    .pg-logos:hover .pg-img { opacity: 0.82; }
    .pg-logos:hover .pg-item:hover .pg-img { opacity: 0.82; scale: 1; filter: none; }
    .pg-block:hover .pg-label-inner { color: rgba(255,255,255,0.45); }
    .pg-block:hover .pg-label::before { background: linear-gradient(90deg, transparent, rgba(255,255,255,0.32)); }
    .pg-block:hover .pg-label::after { background: linear-gradient(270deg, transparent, rgba(255,255,255,0.32)); }
    .pg-label.label-active::before { background: linear-gradient(90deg, transparent, rgba(252,220,80,0.5)) !important; }
    .pg-label.label-active::after { background: linear-gradient(270deg, transparent, rgba(252,220,80,0.5)) !important; }
    .pg-label-inner.label-active { color: rgba(255,255,255,0.85) !important; }
  }
   
  @media (prefers-reduced-motion: reduce) {
    partners-grid *, partners-grid *::before, partners-grid *::after { animation-duration:.001ms !important; animation-iteration-count:1 !important; transition-duration:.001ms !important; }
  }
</style>
<div class="pg">${rowsHTML}</div>`;

    const allLogos = this.querySelectorAll('.pg-logos');
    const allBlocks = this.querySelectorAll('.pg-block');

    const checkBlocks = () => {
      const mid = (document.documentElement.clientHeight || 600) * 0.60;
      allBlocks.forEach(block => {
        const rect = block.getBoundingClientRect();
        const blockMid = (rect.top + rect.bottom) / 2;
        const active = blockMid < mid && blockMid > 0;
        const label = block.querySelector('.pg-label-inner');
        const labelWrap = block.querySelector('.pg-label');
        if (label) label.classList.toggle('label-active', active);
        if (labelWrap) labelWrap.classList.toggle('label-active', active);
      });
    };

     
     
    if (this._scrollAbort) this._scrollAbort.abort();
    this._scrollAbort = new AbortController();
     
     
    let _scrollTick = false;
    const onScroll = () => { if (_scrollTick) return; _scrollTick = true; requestAnimationFrame(() => { checkBlocks(); _scrollTick = false; }); };
    try { document.addEventListener('scroll', onScroll, { passive: true, signal: this._scrollAbort.signal }); checkBlocks(); } catch(e) {}
    ...
    const revelerTout = () => {
      this.querySelectorAll('.pg-img:not(.visible)').forEach(img => img.classList.add('visible'));
    };

    if (this._filet) clearTimeout(this._filet);
    this._filet = setTimeout(revelerTout, 3000);

    if (typeof IntersectionObserver !== 'function') { revelerTout(); return; }

     
    if (this._observer) this._observer.disconnect();
    const observer = this._observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (!entry.isIntersecting) return;
        entry.target.querySelectorAll('.pg-img').forEach((img, i) => {
          setTimeout(() => img.classList.add('visible'), i * 80);
        });
        observer.unobserve(entry.target);
      });
    }, { threshold: 0.15 });
    allLogos.forEach(el => observer.observe(el));
  }
}

customElements.define('partners-grid', PartnersGrid);
}
})();
