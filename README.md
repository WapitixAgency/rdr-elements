...
| `carte.js` | la carte du village ; ses données sont déjà dans le HTML servi, et MapLibre est préchargé avec elle :  | 456.7 Ko, 100.9 Ko brotli |
| `atelier.js` | l'atelier du village (village-atelier, l'outil de l'équipe pour éditer la carte, sur ordinateur) ; le plus gros module du site après l'Espace Rhum (168 Ko brotli), MapLibre préchargé comme pour la carte :  | 761.6 Ko, 173.5 Ko brotli |
...
| `actus.js` | le hub des actualités, ses pages de tag (/medias-actualites/tags/<tag>) et chaque page de post (en-tête et suite de lecture) ; un seul paquet parce qu'on va de l'un à l'autre, leurs charges utiles sont déjà dans le HTML servi : rdr-news, rdr-post-head, rdr-post-more | 282.3 Ko, 53.0 Ko brotli |
...
| `course.js` | les petits modules éditoriaux des anciennes pages Héritage, Bateaux et Terres d'engagements ; leurs charges sont des attributs du HTML servi. La page La course est passée en rdr-course le 25/09 : faits-marquants et manifeste-cartes en sont sortis : heritage-timeline, boat-diagram, pactes-tabs | 47.6 Ko, 11.4 Ko brotli |
...
| `la-course.js` | la page La course (Qu'est-ce que la Route du Rhum, 24/09) : son jeu arrive en attribut jeu, posé par la page maître au rendu serveur :  | 96.0 Ko, 22.6 Ko brotli |
...
Source : `WapitixAgency/route-du-rhum 7b2eabf`.
