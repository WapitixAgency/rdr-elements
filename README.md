...
| `carte.js` | la carte du village ; ses données sont déjà dans le HTML servi, et MapLibre est préchargé avec elle :  | 452.7 Ko, 99.7 Ko brotli |
| `atelier.js` | l'atelier du village (village-atelier, l'outil de l'équipe pour éditer la carte, sur ordinateur) ; le plus gros module du site après l'Espace Rhum (168 Ko brotli), MapLibre préchargé comme pour la carte :  | 757.6 Ko, 172.3 Ko brotli |
...
| `entete.js` | l'en-tête « mode village » (rdr-entete, 22/09), chargé sur toute page qui porte la balise : aujourd'hui la page test de Jules, demain toutes : rdr-entete | 264.1 Ko, 54.2 Ko brotli |
| `apercu.js` | l'aperçu de l'accueil V3 (rdr-accueil-apercu), chargé sur toute page qui porte la balise : rdr-accueil-apercu | 145.3 Ko, 30.0 Ko brotli |
...
| `programme.js` | la Programmation ; sa charge utile (133 Ko) est dans le HTML servi, le module est la page : rdr-programme | 186.6 Ko, 40.0 Ko brotli |
...
| `reglements.js` | les règlements des jeux et concours (23/09) ; les textes sont dans le module, il se dessine seul : rdr-reglements | 57.3 Ko, 13.3 Ko brotli |
| `confidentialite.js` | la politique de confidentialité (24/09) ; le texte est dans le module, il se dessine seul. Pas encore de chemins : l'adresse corrigée de la page n'est pas publiée, le paquet part sur sa balise : rdr-confidentialite | 63.7 Ko, 14.8 Ko brotli |
| `faq.js` | la page FAQ (23/09) : ses données arrivent en attribut jeu, posé par la page maître au rendu serveur :  | 51.4 Ko, 13.1 Ko brotli |
...
| `console.js` | la console de l'équipe (rdr-console, page réservée), chargée sur la présence de sa balise : rdr-console | 128.0 Ko, 29.6 Ko brotli |
...
Source : `WapitixAgency/route-du-rhum 0ebf2f7`.
